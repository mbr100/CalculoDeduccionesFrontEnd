...
@Component({
    ...
    templateUrl: './otros-gastos.html',
    styleUrl: './otros-gastos.css'
})
export class OtrosGastos implements OnInit {
    private route: ActivatedRoute = inject(ActivatedRoute);
    private otrosGastosService: OtrosGastosService = inject(OtrosGastosService);
    ...
    // ---- Datos ----
    public facturas: WritableSignal<FacturaOtrosGastosDTO[]> = signal<FacturaOtrosGastosDTO[]>([]);
    ...
    public loadAll(): void {
        ...
        this.otrosGastosService.getFacturas(this.economicoId).subscribe({
    ...
    public openEditFactura(factura: FacturaOtrosGastosDTO): void {
        this.formFactura.set({
    ...
    public saveFactura(): void {
        ...
        if (this.modalMode() === 'create') {
            const dto: CrearFacturaOtrosGastosDTO = {
            ...
            this.otrosGastosService.crearFactura(dto).subscribe({
                next: (created) => {
            ...
            const dto: ActualizarFacturaOtrosGastosDTO = {
                idFactura: id,
                numeroFactura: form.numeroFactura.trim(),
                proveedor: form.proveedor.trim(),
                descripcion: form.descripcion,
            ...
            this.otrosGastosService.actualizarFactura(dto).subscribe({
                next: (updated) => {
    ...
    public deleteFactura(id: number, numero: string): void {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                this.otrosGastosService.eliminarFactura(id).subscribe({
                    next: () => {
    ...
    public toggleImputaciones(factura: FacturaOtrosGastosDTO): void {
    ...
    public getImporteImputacion(factura: FacturaOtrosGastosDTO, idFase: number): number {
        const imp = factura.imputaciones.find(i => i.idFase === idFase);
        return imp ? imp.importe : 0;
    }

    public getTotalImputado(factura: FacturaOtrosGastosDTO): number {
        return factura.imputaciones.reduce((sum, i) => sum + i.importe, 0);
    }

    public saveImputacion(factura: FacturaOtrosGastosDTO, idFase: number, value: number): void {
        const key = `${factura.idFactura}-${idFase}`;
        this.savingImputaciones.update(s => ({...s, [key]: 'saving'}));

        const dto: ActualizarImputacionOtrosGastosFaseDTO = {
            idFactura: factura.idFactura,
            idFase,
            importe: value || 0
        };
        this.otrosGastosService.actualizarImputacion(dto).subscribe({
            next: () => {
                this.otrosGastosService.getFacturas(this.economicoId).subscribe({
                    next: (data) => {
